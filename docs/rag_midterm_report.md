...
| **Android 客户端** | 2026-05-24~25 | Kotlin + Jetpack Compose、SSE 消费、ProductDetailScreen、悬浮 Agent 入口 |
...
### 7.3 QueryPlan 基础设施（向 LLM 结构化意图升级的准备阶段）

> **状态：基础设施已实现，不接管实际检索（2026-06-02）**
...
→ QueryUnderstandingService 融合 LLM plan + 旧规则 fallback
...
| `QueryUnderstandingService` | 编排 legacy QueryAnalyzer + LLMQueryPlanner shadow compare，plannerUsedForRetrieval=false |
...
**重要承诺**：
- **第二阶段为 shadow mode**，不接管真实检索，不影响 `/api/chat/stream`
- **默认 planner enabled=false**（通过 `QUERY_PLANNER_ENABLED` 环境变量开启）
- **QueryAnalyzer / RetrievalRouter / QueryRewriteService 行为完全不受影响**
- **第三阶段将进行 QueryPlan 与 legacy QueryAnalyzer 对比评估 / 局部接管**
...
| 7 | LLM Query Rewrite | 已实现（2026-06-02，SoftSemanticLexicon + LLMQueryRewriter + QueryRewriteService，默认 disabled） |
| 8 | 用户评价 / FAQ 独立向量化 | 已实现（2026-06-02，REVIEW_SUMMARY / FAQ / MARKETING_COPY chunk 已接入） |
| 9 | QueryPlan 基础设施 | 已实现（2026-06-02，CatalogTaxonomyService + QueryPlan + QueryPlanValidator，不接管检索） |
| 10 | LLMQueryPlanner（LLM 填充 QueryPlan） | 已实现（2026-06-02，shadow mode，plannerUsedForRetrieval=false，默认 disabled） |
| 11 | QueryPlan 接管检索评估/局部接管 | 已实现（2026-06-02，assist/takeover mode，QueryPlanToAnalysisMapper + QueryPlanGatingService + effectiveAnalysis 流） |
| 12 | Planner vs Legacy 系统评估 / ChatService 集成 | 第四阶段 A+B 已完成（ChatService Shadow + Assist 局部接管），下一阶段做系统评估 + 缓存优化 |
...
| QueryPlan 基础设施 | 已实现 CatalogTaxonomyService + QueryPlan + QueryPlanValidator + LLMQueryPlanner（shadow mode），plannerUsedForRetrieval=false |
