...
> 已实现商品相关 API、SSE 对话接口和 TTS 语音合成接口。支持 Mock LLM 与 Doubao LLM 两种模式。
...
- **认证方式**: Demo 登录 + Bearer Token（TTS 接口可选认证）
...
event:product_card
data:{"product_id":"p_beauty_010","name":"芙丽芳丝净润洗面霜","price":89.0,"currency":"CNY","image_url":"/images/p_beauty_010.jpg","reason":"匹配「温和洁面」需求，适合日常护肤。"}
...
> - product_card.reason 由后端 `RecommendationReasonService` 基于 softKeywords / positiveKeywords / 商品真实字段 / 价格约束确定性生成，不调用 LLM 逐卡片生成
> - 不再返回占位文本如"由LLM推荐"或"该商品来自当前商品库候选结果，具体推荐理由将在后续 LLM 阶段生成"
...
> **认证说明**: `/api/cart/**` 为强认证接口，未登录返回 401 `{"code":"INVALID_TOKEN","message":"未登录或 token 已过期"}`。`/api/chat/stream` 为可选认证：不传 token 时普通聊天可用；传有效 token 时启用对话式加购等需要用户身份的功能；传无效 token 时普通推荐不报 500，对话式加购返回"请先登录"提示。
>
> **字段命名**: 所有请求体和响应体统一使用 snake_case。登录响应字段为 access_token/user_id/expires_in/token_type/username，CartView/CartItem 所有字段为 snake_case。
>
> **购物车写操作**: POST/PATCH/DELETE 成功后全部返回完整 CartView，前端不重算 subtotal/total_amount。done 事件后前端自动调用 GET /api/cart 刷新角标。
>
> **SSE 协议**: data 字段为单行 JSON，每次响应必须以 done 或 error 结束。product_card data 字段均为 snake_case (product_id/image_url 等)。done data 为 {}。

---

## 5.2 购物车 API

> 所有购物车接口需要 `Authorization: Bearer {token}` 头部。未登录返回 401。
  ...
  "items": [
    {
      ...
      "price": 899.0,
      "currency": "CNY",
      "image_url": "/images/p_clothes_007.jpg",
      "quantity": 2,
      "subtotal": 1798.0
    }
  ],
...
**Response (200)**: CartView（同上）

**Response (404)**:
```json
{"code": "PRODUCT_NOT_FOUND", "message": "商品不存在: p_xxx"}
```

**Response (400)**:
```json
{"code": "INVALID_QUANTITY", "message": "数量必须在 1 到 99 之间"}
```
...
> **认证说明**: `/api/chat/stream` 的 `Authorization` header 为可选。不传 token 时普通聊天仍可用；传有效 token 时可使用对话式加购功能。具体用法：在 `/api/chat/stream` 请求中添加 `Authorization: Bearer {token}` 头部即可启用对话式加购。

Agent 对话中支持 ADD_TO_CART intent。识别规则：
- "加入购物车"、"加到购物车"、"加购"
- "买这个"、"把第一款加入购物车"、"把第二个加到购物车"

目标商品解析优先级：
1. `page_context.current_product_id`（商品详情页）
2. "第一款/第二款/第三款" → 从 ConversationState.recommendedProductIds 取对应项
3. "这个/这款/它" → 最近一次推荐的第一张 product_card
4. 无法确定时 → 返回澄清提示，不执行加购

未登录时返回："请先登录后再加入购物车。"
成功时通过 text SSE 返回："已加入购物车：{商品名}，¥{price}，数量 ×{qty}"

> 对话式加购不生成 product_card，不影响现有推荐行为。

### 购物车意图理解

`/api/chat/stream` 支持以下购物车相关语义：购物车语义采用 **Semantic Frame + Rule Match + LLM Assist Slot Filling** 方式理解。

| 用户意图 | 语义框架 | 示例 | SSE 响应 |
|---------|---------|------|---------|
| CART_SUMMARY | `cart.summary` | "当前已经买了多少钱了" | text + done |
| AMOUNT_GAP_QUERY | `cart.amount_gap_query` | "离2000还差多少" | text（差额）+ done（**不发送 product_card**） |
| COMPLETION_RECOMMEND | `cart.completion_recommend` | "如果要凑1000块" | text + product_card(s) + done |
| 凑单澄清 | `cart.completion_clarify` | "凑单推荐一下" | text（追问目标金额）+ done |

- 购物车金额由 CartService 真实计算，LLM 不编造金额
- 凑单商品由 ProductService 真实检索，LLM 不编造 product_id
- 未登录用户访问购物车意图时返回"请先登录"提示
- targetAmount 缺失时返回追问目标金额
- `AMOUNT_GAP_QUERY` 与 `COMPLETION_RECOMMEND` 的执行区别：前者只报告差额（text），后者推荐商品（text + product_card）
- 规则层（CartSemanticFrameMatcher）负责发现和提示，LLM 只能在预定义语义框架中选择并填槽
- SSE 协议不变，仍为 text/product_card/done

---

## 5.3 TTS 语音合成

> ASR 当前由 Android 原生 SpeechRecognizer 实现，识别结果直接作为普通文本走 `/api/chat/stream`，**不需要后端 ASR 接口**。

### POST /api/tts/speak

将 AI 回复文本合成为语音，返回可播放的音频 URL。
...
| 字段 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| text | string | 是 | — | 待合成文本，trim 后不能为空，超过 maxTextLength 后端自动截断 |
| voice | string | 否 | zh-CN-XiaoxiaoNeural | 语音角色，必须在 allowedVoices 白名单内 |
| format | string | 否 | mp3 | 音频格式，当前仅支持 mp3 |
...
```json
{
  "audio_url": "http://localhost:8080/api/tts/audio/tts_abc123.mp3",
...
| HTTP 状态 | code | 说明 |
|-----------|------|------|
| 400 | INVALID_TTS_TEXT | 文本不能为空 |
| 400 | INVALID_TTS_VOICE | 不支持的语音角色 |
| 400 | INVALID_TTS_FORMAT | 当前仅支持 mp3 |
| 503 | TTS_PROVIDER_UNAVAILABLE | 当前 TTS 服务不可用（如 edge-tts 未安装或 TTS_ENABLED=false） |
| 502 | TTS_SYNTHESIS_FAILED | 语音合成失败，请稍后重试 |

> **降级说明**: TTS 失败不影响 `/api/chat/stream` 文字聊天主链路，前端自动降级为不播放语音。
...
| 参数 | 类型 | 说明 |
|------|------|------|
| fileName | string | 音频文件名，格式必须为 `tts_[a-zA-Z0-9_-]+.mp3` |
...
> - `plannerUsedForRetrieval` 在 shadow/disabled 模式下为 false；assist 模式下 gate 通过时为 true
...
| INVALID_TTS_TEXT | 400 | TTS 文本不能为空 |
| INVALID_TTS_VOICE | 400 | 不支持的语音角色 |
| INVALID_TTS_FORMAT | 400 | 当前仅支持 mp3 |
| TTS_PROVIDER_UNAVAILABLE | 503 | 当前 TTS 服务不可用 |
| TTS_SYNTHESIS_FAILED | 502 | 语音合成失败 |
