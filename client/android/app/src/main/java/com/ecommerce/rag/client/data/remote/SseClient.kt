...
class SseClient(
    ...
    suspend fun streamChat(
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                // 阶段 9：可选 Authorization。
                // - ChatViewModel 调用不传 token，仍兼容；
                // - AssistantViewModel 调用读 AuthTokenStore，让 Agent 对话式加购可用；
                // - 即使后端 /api/chat/stream 是可选登录，带上也无害。
    ...
    private fun PageType.toWireName(): String = when (this) {
        ...
        PageType.CHAT -> "CHAT"
        // 阶段 10：新增个人中心 / 购物车 / 登录页类型，后端若未识别按 UNKNOWN 兜底即可。
