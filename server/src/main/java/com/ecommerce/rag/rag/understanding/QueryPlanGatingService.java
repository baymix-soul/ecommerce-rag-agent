...
public class QueryPlanGatingService {
    ...
    public QueryPlanGateDecision decide(QueryPlanningResult planningResult,
        ...
        // CART_TOP_UP with null targetAmount: don't execute recommendation, return clarification
        if (QueryPlan.INTENT_CART_TOP_UP.equals(intent) || QueryPlan.INTENT_CART_COMPLETION_RECOMMEND.equals(intent)) {
            CartPlan cartPlan = validatedPlan.getCart();
            if (cartPlan != null && cartPlan.getTargetAmount() == null
                    && (validatedPlan.getNeedsClarification() == null || validatedPlan.getNeedsClarification())) {
                // Allow through but mark for clarification handling
                decision.setAllowed(true);
                decision.setSelectedSource(QueryPlanGateDecision.SELECTED_PLANNER);
                decision.setReason("cart intent with null targetAmount, will return clarification");
