...
public class RagPromptBuilder {
    ...
    public String build(String message, List<ChatCandidate> candidates,
        ...
        if (candidates == null || candidates.isEmpty()) {
            ...
            PerfTraceContext.addAttribute("prompt_candidate_count", 0);
            PerfTraceContext.addAttribute("prompt_length", prompt.length());
            PerfTraceContext.addAttribute("response_style", responseStyle);
            return prompt;
        }
        ...
        String prompt = SYSTEM_TEMPLATE + "\n" + outputRule + "\n" +
                ...
                "候选商品列表：\n" + candidateSection;
        PerfTraceContext.endSpan("prompt.build");
        PerfTraceContext.addAttribute("prompt_candidate_count", candidates.size());
        PerfTraceContext.addAttribute("prompt_length", prompt.length());
        PerfTraceContext.addAttribute("response_style", responseStyle);
        return prompt;
    }
