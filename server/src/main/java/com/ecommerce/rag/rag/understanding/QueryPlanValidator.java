...
public class QueryPlanValidator {
    ...
    private static final Set<String> CART_INTENTS = Set.of(
            QueryPlan.INTENT_CART_SUMMARY, QueryPlan.INTENT_CART_TOP_UP,
            QueryPlan.INTENT_CART_COMPLETION_RECOMMEND
    );
    ...
    public QueryPlanValidationResult validate(QueryPlan plan, CatalogTaxonomySnapshot taxonomy) {
        ...
        validateCartPlan(validated, warnings, errors);
    ...
    private void validateCartPlan(QueryPlan plan, List<String> warnings, List<String> errors) {
        String intent = plan.getIntent();
        if (intent == null || !CART_INTENTS.contains(intent)) return;

        CartPlan cart = plan.getCart();
        if (cart == null) {
            warnings.add("cart intent '" + intent + "' but cart plan is null, auto-creating");
            cart = new CartPlan();
            plan.setCart(cart);
        }

        if (QueryPlan.INTENT_CART_SUMMARY.equals(intent)) {
            if (cart.getNeedsCart() == null || !cart.getNeedsCart()) {
                cart.setNeedsCart(true);
                warnings.add("cart.needsCart forced to true for CART_SUMMARY");
            }
            if (cart.getNeedsRecommendation() != null && cart.getNeedsRecommendation()) {
                cart.setNeedsRecommendation(false);
                warnings.add("cart.needsRecommendation forced to false for CART_SUMMARY");
            }
        }

        if (QueryPlan.INTENT_CART_TOP_UP.equals(intent)
                || QueryPlan.INTENT_CART_COMPLETION_RECOMMEND.equals(intent)) {
            if (cart.getNeedsCart() == null || !cart.getNeedsCart()) {
                cart.setNeedsCart(true);
                warnings.add("cart.needsCart forced to true for " + intent);
            }

            String action = cart.getAction();
            if (CartPlan.ACTION_AMOUNT_GAP_QUERY.equals(action)) {
                if (cart.getNeedsRecommendation() != null && cart.getNeedsRecommendation()) {
                    cart.setNeedsRecommendation(false);
                    warnings.add("cart.needsRecommendation forced to false for AMOUNT_GAP_QUERY");
                }
            } else if (CartPlan.ACTION_COMPLETION_RECOMMEND.equals(action)) {
                if (cart.getNeedsRecommendation() == null || !cart.getNeedsRecommendation()) {
                    cart.setNeedsRecommendation(true);
                    warnings.add("cart.needsRecommendation forced to true for COMPLETION_RECOMMEND");
                }
            } else {
                if (cart.getNeedsRecommendation() == null || !cart.getNeedsRecommendation()) {
                    cart.setNeedsRecommendation(true);
                    warnings.add("cart.needsRecommendation forced to true for " + intent);
                }
            }

            if (cart.getTargetAmount() != null) {
                if (cart.getTargetAmount().compareTo(BigDecimal.ZERO) <= 0) {
                    errors.add("cart.targetAmount must be > 0 for " + intent + ", got " + cart.getTargetAmount());
                }
                if (cart.getTargetAmount().compareTo(new BigDecimal("100000")) > 0) {
                    warnings.add("cart.targetAmount (" + cart.getTargetAmount() + ") is very large, may be unreasonable");
                }
            } else {
                if (plan.getNeedsClarification() == null || !plan.getNeedsClarification()) {
                    plan.setNeedsClarification(true);
                    warnings.add("targetAmount is null for " + intent + ", set needsClarification=true");
                }
            }
        }

        if (cart.getReferencedProductIds() != null && !cart.getReferencedProductIds().isEmpty()) {
            for (String pid : cart.getReferencedProductIds()) {
                if (!PRODUCT_ID_PATTERN.matcher(pid).matches()) {
                    warnings.add("cart.referencedProductIds contains unknown format: '" + pid + "'");
                }
            }
        }

        if (cart.getCurrency() == null || cart.getCurrency().isBlank()) {
