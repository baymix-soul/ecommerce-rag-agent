...
- [x] Cart-aware Query Understanding: 购物车意图理解（CART_SUMMARY/CART_TOP_UP/CART_COMPLETION_RECOMMEND）
- [x] CartSemanticHintService: 语义词表 hint 层
- [x] CartTopUpRecommendationService: 凑单推荐服务
- [x] Cart Semantic Understanding Refactor: CartSemanticFrame 体系（semantic frames + rule match + LLM assist slot filling）
- [x] CartSemanticFrameMatcher: EXACT/PARTIAL/NONE 匹配（反模式排除：推荐2000元以内的电脑≠购物车）
- [x] CartPlan.ACTION_AMOUNT_GAP_QUERY: 差额查询（仅计算差额，不推荐商品）vs COMPLETION_RECOMMEND（推荐商品）

### 基础搭建

- [x] [P1] Backend-Frontend Contract Alignment：接口契约对齐（错误码、snake_case、认证模型、SSE 协议、购物车 CartView）—— 2026-06-05
- [x] [P1] 新增合约测试：FrontendContractAuthTest(5) + FrontendContractCartTest(13) + FrontendContractChatStreamTest(10)
- [x] [P1] 更新 docs/api_spec.md 对齐前端接口文档
...
- [x] [P2] 多轮对话上下文管理
- [x] [P2] 排除已推荐商品逻辑
- [x] [P2] 用户登录校验 + Redis 购物车 + 对话式加购（2026-06-04）
- [x] [P1] Auth Bugfix: 修复对话式加购 token 不生效问题（2026-06-05）
- [x] [P2] Redis 接入（Spring Data Redis + Docker Compose）
- [x] [P1] Backend Voice/TTS: POST /api/tts/speak + GET /api/tts/audio/{fileName}（2026-06-07）
- [x] [P1] Backend Recommendation Reason: 替换 product_card.reason 占位文本，基于 softKeywords / 商品字段 / 价格约束确定性生成（2026-06-08）
- [ ] [P3] 购物车增强功能
- [ ] [P3] 多模态输入（图片搜索）
