...
## [2026-06-08] Backend Recommendation Reason: 替换 product_card.reason 占位文本

### 新增
- `RecommendationReasonService`：确定性推荐理由生成服务，不调用 LLM
  - `generateReason(ChatCandidate, Product, QueryAnalysisResult)`：基于 softKeywords / positiveKeywords / 商品真实字段 / 价格约束生成一句话 reason
  - `generateTopUpReason(Product, gap, targetAmount)`：凑单推荐卡片 reason
  - `isPlaceholder(String)`：检测占位文本（"由LLM推荐"等）
- 关键词匹配逻辑：收集 softKeywords + positiveKeywords，与商品 name/brand/category/subCategory/description/specs/reviewSummary/marketingCopy 做文本匹配
- 生成规则：匹配关键词 → `匹配「{kw}」需求，适合{scenario}。`；含价格约束 → 追加 `同时满足 ¥{maxPrice} 以下预算。`
- fallback 层级：subCategory → category+maxPrice → brand → `符合当前检索条件，可优先查看。`

### 修改
- `ChatService.sendProductCards()`：发送 product_card 前调用 `RecommendationReasonService.generateReason()` 替换 reason，移除 `REASON_PLACEHOLDER` 常量
- `ChatService` 构造函数新增 `RecommendationReasonService` 参数
- `CartTopUpRecommendationService`：凑单卡片 reason 从硬编码 `"适合凑单"` 改为调用 `generateTopUpReason()`，构造函数新增参数
- `docs/api_spec.md`：SSE 示例中 product_card.reason 更新为真实示例，新增 reason 生成说明
- `docs/rag_design.md`：模块划分表新增 RecommendationReasonService，SSE 事件流章节新增 reason 生成规则说明

### 测试
- `RecommendationReasonServiceTest`（11 个用例）：softKeywords 命中、多关键词最多 2 个、空 softKeywords fallback、maxPrice 预算提示、不含 productId、非占位、长度不超 60、不强行编造关键词、程序员电脑场景、跑鞋轻量场景、凑单 reason
- `ChatServiceProductCardReasonTest`（8 个用例）：reason 非空、非占位、使用 softKeywords、多卡片都有 reason、maxPrice 提到预算、recovery card 非占位、JSON 结构不变、isPlaceholder 检测
- `CartTopUpProductCardReasonTest`（3 个用例）：reason 非占位、含凑单提示、不调用 LLM
- 修复 5 个已有测试的构造函数参数（ChatServiceCartAwareTest / ChatServiceCartSemanticTest / ChatServiceContextNoMatchRecoveryTest / ChatServicePerfInstrumentationTest / CartTopUpRecommendationServiceTest）

### 设计原则
- 不调用 LLM 生成 reason，保证低延迟
- reason 必须基于商品真实字段，不编造功能/材质/功效
- reason 长度控制在 12-36 字，最多不超过 60 字
- 不修改 Android/client、不修改 SSE 协议、不修改 product_card 字段结构
- 凑单推荐也使用真实 reason，不再是硬编码占位

---

## [2026-06-07] Backend Voice/TTS: 实现 POST /api/tts/speak + GET /api/tts/audio/{fileName}

### 新增
- TtsSpeakRequest / TtsSpeakResponse DTO：TTS 请求/响应模型（text/voice/format/audio_url/content_type）
- TtsProvider 接口 + TtsAudioResult / StoredTtsAudio：TTS 提供者抽象层
- EdgeTtsCliProvider：ProcessBuilder 调用 edge-tts CLI 生成 mp3，参数数组防注入，超时保护，临时文件清理
- TtsAudioStore：音频文件存储管理（安全文件名校验、路径穿越防护、24h 自动清理、UUID 命名）
- TtsService：TTS 业务逻辑（text 校验截断、voice 白名单、format 校验、audio_url 构造）
- TtsController：POST /api/tts/speak（返回 audio_url）+ GET /api/tts/audio/{fileName}（返回 audio/mpeg）
- TtsException + GlobalExceptionHandler 处理：INVALID_TTS_TEXT(400) / INVALID_TTS_VOICE(400) / INVALID_TTS_FORMAT(400) / TTS_PROVIDER_UNAVAILABLE(503) / TTS_SYNTHESIS_FAILED(502)
- AppProperties.TtsProperties + application.yml tts 配置节：enabled/provider/audio-dir/max-text-length/default-voice/allowed-voices/default-format/command-timeout-seconds

### 修改
- Dockerfile：runtime stage 安装 python3 + pip + edge-tts CLI
- deploy/.env.production.example：新增 TTS 环境变量模板

### 测试
- TtsRequestValidationTest（10 个用例）：text 空/超长截断、voice 空/非法、format mp3/wav、tts disabled
- TtsAudioStoreTest（7 个用例）：save 生成安全文件名、load 返回资源、非法文件名拒绝、路径穿越防护、cleanup 不删新文件
- TtsControllerTest（10 个用例）：audio_url 构造、host 使用 request host、X-Forwarded 头支持、GET 返回 audio/mpeg、404、provider 不可用 503、合成失败 502
- EdgeTtsCliProviderTest（5 个用例）：providerName、available 检测、unavailable 时抛异常、ProcessBuilder 设计验证

### 设计原则
- TTS 失败不影响 /api/chat/stream 主链路，前端自动降级
- Authorization 可选，不强制登录
- audio_url 根据请求 host 构造，支持 X-Forwarded-* 反向代理头
- 不返回 127.0.0.1（Android 无法访问），优先使用 request.getServerName()
- 不引入复杂流式音频，优先 audio_url 方案
- 不修改 Android/client、不修改 SSE 协议、不修改 /api/chat/stream

---

## [2026-06-07] Backend Cart Semantic Understanding Refactor: Semantic Frame 体系

### 新增
- CartSemanticFrame：预设购物车语义框架数据模型（frameId/description/intent/cartAction/requiredSlots/positiveExamples/negativeExamples）
- CartSemanticFrameCatalog：语义框架目录（cart.summary/cart.amount_gap_query/cart.completion_recommend/cart.completion_clarify/cart.add_item）
- CartSemanticMatchResult：语义框架匹配结果（EXACT/PARTIAL/NONE matchLevel, extractedSlots, missingSlots, ruleConfidence）
- CartSemanticFrameMatcher：语义框架匹配器，负责规则层发现和提示，输出匹配结果
- CartPlan.ACTION_AMOUNT_GAP_QUERY：新增「仅计算差额不推荐商品」的购物车动作
- ChatService.handleAmountGapQuery：差额查询单独处理（text + done，无 product_card）
- QueryUnderstandingResult.cartSemanticMatch：查询理解结果中关联语义匹配结果

### 修改
- CartPlan：移除 VIEW_CART/TOP_UP/VIEW_CART_ITEMS，统一为 AMOUNT_GAP_QUERY/COMPLETION_RECOMMEND/CART_SUMMARY/ADD_TO_CART/REMOVE_FROM_CART
- QueryPlannerPromptBuilder：从 CartSemanticHintService 重构为 CartSemanticFrameCatalog + CartSemanticFrameMatcher，prompt 注入语义框架列表和匹配结果
- QueryUnderstandingService：集成 CartSemanticFrameMatcher，在 planner 调用前执行语义匹配
- QueryPlanValidator.validateCartPlan()：区分 AMOUNT_GAP_QUERY（needsRecommendation=false）和 COMPLETION_RECOMMEND（needsRecommendation=true）
- ChatService.handleCartIntentFromPlan()：区分 AMOUNT_GAP_QUERY/COMPLETION_RECOMMEND/ADD_TO_CART 执行路径
- CartSemanticFrameMatcher：null-safe hintService 处理，多帧匹配时解析 targetAmount

### 测试
- CartSemanticFrameMatcherTest（14 个用例）：EXACT/PARTIAL/NONE 匹配、反例排除、金额解析
- QueryPlannerPromptSemanticFrameTest（7 个用例）：prompt 包含语义框架、匹配结果、反例排除
- QueryUnderstandingCartAssistTest（5 个用例）：语义匹配结果注入、planner 错误降级
- ChatServiceCartSemanticTest（8 个用例）：AMOUNT_GAP_QUERY/COMPLETION_RECOMMEND、未登录、ADD_TO_CART 不受影响

### 设计原则
- 规则层负责匹配和提示（CartSemanticFrameMatcher），不作为最终判断器
- LLM 只能在预定义 semantic frames 中选择并填槽
- 购物车金额由 CartService 计算，凑单商品由 ProductService 产生
- 普通商品搜索不受影响，SSE 协议不变

## [2026-06-06] Backend Cart-aware Query Understanding: 购物车意图理解与凑单推荐

### 新增
- CartPlan 子结构：表达购物车意图的结构化字段（action/targetAmount/currency/needsCart/needsRecommendation）
- CartSemanticHintService：语义词表 hint 层，提供购物车语义 hint 给 LLM planner
- CartSemanticHint：hint 输出结构（cartRelated/possibleIntents/ruleParsedTargetAmount/keywords）
- CartTopUpRecommendationService：基于真实购物车和商品库的凑单推荐服务
- RetrievalIntent 新增 CART_SUMMARY/CART_TOP_UP/CART_COMPLETION_RECOMMEND
- QueryPlan 新增 cart 字段和购物车 intent 常量
- QueryPlannerPromptBuilder 新增购物车能力说明、示例、规则
- QueryPlanValidator 新增购物车 plan 校验逻辑
- QueryPlanGatingService 允许购物车 intent 进入 planner
- ChatService 新增 handleCartSummary/handleCartTopUp
- AppProperties.CartProperties + cart.top-up-tolerance 配置

### 修改
- RetrievalRouter 新增 isCartSummary/isCartTopUp 语义识别
- QueryPlan 新增 cart 字段、CART_SUMMARY/CART_TOP_UP/CART_COMPLETION_RECOMMEND intent 常量
- QueryPlannerPromptBuilder 注入 CartSemanticHintService，prompt 包含购物车语义 hint
- QueryPlanValidator 新增 validateCartPlan 校验
- QueryPlanGatingService 新增购物车 intent 接管条件
- ChatService 注入 CartTopUpRecommendationService，新增购物车意图处理流程

### 测试
- QueryPlannerPromptCartIntentTest (7 tests)
- CartSemanticHintServiceTest (6 tests)
- QueryPlanValidatorCartTest (6 tests)
- QueryUnderstandingCartPlannerTest (5 tests)
- CartTopUpRecommendationServiceTest (7 tests)
- ChatServiceCartAwareTest (8 tests)

---

## [2026-06-05] Backend-Frontend Contract Alignment: 接口契约对齐

- **类型**: feat(server)
- **描述**: 按 frontend-api-spec.md 对齐后端接口契约，完成 App 端冒烟测试前的后端适配
  - 错误码对齐：AuthInterceptor 401 响应 code `UNAUTHORIZED` → `INVALID_TOKEN`，AuthController/GlobalExceptionHandler login 失败 code `AUTH_FAILED` → `INVALID_CREDENTIALS`
  - ProductNotFoundException 消息中文化：`"Product not found: xxx"` → `"商品不存在: xxx"`
  - DTO snake_case 验证通过（LoginRequest/LoginResponse/CartView/CartItem/AddCartItemRequest/Product/ProductCard 均已使用 @JsonProperty）
  - 认证模型：`/api/cart/**` 强认证（无 token → 401 INVALID_TOKEN），`/api/chat/stream` 可选认证（无 token → 正常推荐）
  - 新增合约测试：
    - FrontendContractAuthTest(5): 登录字段 snake_case、无 camelCase、401 错误码、demo/demo123 可用
    - FrontendContractCartTest(13): 强认证 401、空购物车 CartView、POST/PATCH/DELETE 返回 CartView、snake_case、quantity 验证
    - FrontendContractChatStreamTest(10): 可选认证、SSE 单行 JSON、done={}、page_context null 不 400、加购不发送 product_card、每流以 done/error 结束
  - 更新 docs/api_spec.md 文档说明
  - 不修改 Android 代码、不修改 RAG 检索链路、不修改 product_card 事件结构
- **影响范围**: AuthInterceptor, AuthController, GlobalExceptionHandler, ProductNotFoundException, docs/api_spec.md, 新增 3 个测试文件
- **关联决策**: D-001

## [2026-06-05] Auth Bugfix: 修复对话式加购"请先登录"问题

- **类型**: fix(server)
- **描述**: 修复 `/api/chat/stream` 携带 valid token 时对话式加购仍提示"请先登录"的问题
  - 根因：AuthInterceptor 仅解析 `/api/cart/**` 路径的 Authorization header，`/api/chat/stream` 被直接放行，CurrentUserContext 从未设置
  - 修复 AuthInterceptor：对所有路径全局解析 Authorization header，仅对 `/api/cart/**` 返回 401（强认证），对 `/api/chat/stream` 为可选认证
  - 新增 AuthContextSnapshot：不可变 value object，在请求线程捕获 userId，显式传递给 ChatService，消除 ThreadLocal 在异步线程中丢失的风险
  - ChatController 在请求线程捕获 AuthContextSnapshot，传给 ChatService.chat(request, authContext)
  - ChatService 保留 chat(ChatRequest) 单参重载（向后兼容，默认未认证），双参重载接收 AuthContextSnapshot
  - handleAddToCart/handleViewCart/handleRemoveFromCart 全部改用 AuthContextSnapshot 参数，不再依赖 CurrentUserContext
  - 新增 AuthInterceptorOptionalAuthTest(9)、ChatServiceAddToCartAuthContextTest(5)、ChatControllerChatAuthIntegrationTest(3)
  - 更新 ChatServiceAddToCartTest（+2 个 auth context 测试）
  - 不修改 Android 代码、不修改 SSE 协议、不修改 product_card 格式
- **影响范围**: AuthInterceptor, AuthContextSnapshot(新), ChatController, ChatService, 测试文件, docs/api_spec.md
- **关联决策**: D-001

---

## [2026-06-04] Backend + Android 增强阶段一：用户登录校验 + Redis 购物车 + 对话式加购

- **类型**: feat(server) + feat(client)
- **描述**: 完成用户认证与购物车最小闭环，支持 demo 登录和对话式加购
  - 新增 Redis 依赖（Spring Data Redis），Docker Compose 新增 redis 服务，本地开发可通过 REDIS_ENABLED=false 禁用
  - 新增 Auth 模块：POST /api/auth/login，SecureRandom token，Redis/in-memory token store，TTL 7 天
  - 新增 AuthInterceptor（OncePerRequestFilter），保护 /api/cart/** 接口
  - 新增 Cart 模块：CartView/CartItem DTO、CartService + RedisCartStore/InMemoryCartStore、5 个 REST API
  - 新增 ADD_TO_CART/VIEW_CART/REMOVE_FROM_CART intent（RetrievalRouter）
  - ChatService 新增 handleAddToCart/handleViewCart/handleRemoveFromCart，支持商品 ID 解析优先级
  - Android 新增 LoginApiClient + AuthTokenStore + CartApiClient + CartViewModel + CartScreen
  - Android MainActivity 集成 Demo 自动登录 + 购物车角标 + 商品详情页真实加购按钮
  - 新增 37 个后端测试：AuthServiceTest(8) + CartServiceTest(12) + CartControllerTest(8) + ChatServiceAddToCartTest(9)
  - 更新 smoke_test.sh（+3 项测试）、docker-compose.yml（+redis 服务）、.env.production.example（+Redis/Auth 变量）
  - 更新 docs/api_spec.md、docs/deployment_mvp.md、docs/rag_design.md
  - 不改现有 RAG 推荐安全原则，不让 LLM 编造商品 ID，不修改 product_card 结构，不破坏 SSE 协议
  - 不接入真实支付，不做复杂注册/权限系统，不提交真实 API Key/密码
- **影响范围**: server/pom.xml, application.yml, AppProperties, Auth*, Cart*, ChatService, RetrievalRouter/Intent, GlobalExceptionHandler, docker-compose, .env.production.example, smoke_test.sh, Android *, docs/*
- **关联决策**: D-001
